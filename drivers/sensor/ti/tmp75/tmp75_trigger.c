...
LOG_MODULE_DECLARE(TMP75, CONFIG_SENSOR_LOG_LEVEL);
 ...
 * @param pins - not used
 */
void tmp75_trigger_handle_alert(const struct device *gpio, struct gpio_callback *cb,
				gpio_port_pins_t pins)
{
	struct tmp75_data *drv_data = CONTAINER_OF(cb, struct tmp75_data, temp_alert_gpio_cb);
	/* Successful read, call set callbacks */
	if (drv_data->temp_alert_handler) {
		drv_data->temp_alert_handler(drv_data->tmp75_dev, drv_data->temp_alert_trigger);
	}
}
